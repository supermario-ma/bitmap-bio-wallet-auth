...
async function disconnect(){
  if(!session)return;
  // Best effort: the server row is what actually grants editing rights.
...
function init(){
  ...
  document.body.append(dialog);$('bio-editor-close').addEventListener('click',()=>dialog.close());$('bio-editor-disconnect').addEventListener('click',async()=>{if(busy)return;await disconnect();status('Disconnected. Sign again to edit.',true)});dialog.addEventListener('close',()=>{if(!busy)disconnect()});dialog.addEventListener('cancel',()=>{if(busy)status('You may reopen the editor to check the result.')});
