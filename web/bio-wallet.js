...
function init(){
  ...
  document.body.append(dialog);$('bio-editor-close').addEventListener('click',()=>dialog.close());$('bio-editor-disconnect').addEventListener('click',()=>disconnect());dialog.addEventListener('close',()=>{if(!busy)disconnect()});dialog.addEventListener('cancel',()=>{if(busy)status('You may reopen the editor to check the result.')});
